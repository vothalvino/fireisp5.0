...
function validateEnv(logger) {
  ...
  // WireGuard: fatal in production when server mode is enabled without a
  // reachable endpoint (the app cannot infer its own public host). The server +
  // client public keys are AUTO-GENERATED by the host bootstrap on first boot and
  // persisted to the WG_KEY_DIR volume, so they are only a warning when unset —
  // set them explicitly only to pin an externally-managed keypair.

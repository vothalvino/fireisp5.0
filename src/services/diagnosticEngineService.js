...
/**
 * Resolve the `devices.id` monitored for a client's active contract's CPE.
 * Returns null if there is no active contract or no monitored CPE.
 */
async function _resolveClientDeviceId(clientId, orgId) {
...
async function _diagSlowFiber(clientId, orgId) {
  ...
  try {
    const deviceId = await _resolveClientDeviceId(clientId, orgId);

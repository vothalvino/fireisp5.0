...
│   ├── schema.sql           # Combined schema (all 225 tables + column additions)
...
| 226 | `queue_tree_nodes` | Hierarchical queue tree node definitions (§10.1) — parent_id self-FK for tree structure, queue_type ENUM (tree/simple), NAS interface, max/burst/threshold limits, burst_time_seconds, queue_kind, sort_order; exportable as MikroTik RouterOS script |

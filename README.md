...
│   ├── schema.sql           # Combined schema (all 328 tables + column additions)
│   └── migrations/          # Individual numbered migration files (001–399)
...
> **Migration 386 — Per-organization outbound email (SMTP) configuration:** Adds `organization_email_settings` (one row per org, mirrors migration 167's `organization_database_configs` pattern exactly — AES-256-GCM `smtp_password_encrypted` via `src/utils/encryption.js`, never returned in any response; `GET /email-settings` exposes only `configured: boolean`). `PUT /email-settings` uses the same write-only three-state password contract as `ai.js`'s provider `api_key` (field omitted → keep existing; `""` → clear to NULL; a value → re-encrypt and replace). `POST /email-settings/test` sends a real test email and records `last_test_at`/`last_test_status`/`last_test_error`. New permissions `email_settings.view`/`email_settings.update`, granted to admin + super_admin ONLY (excluded even from readonly/auditor's blanket `*.view` wildcard, mirroring migration 377's api_tokens/webhooks carve-out — an SMTP credential is org-wide send-as-anyone infrastructure with no natural business-role owner). Also makes `emailTransport.sendEmail({ organizationId, ... })` org-aware: it now loads and caches (`invalidateOrgTransport()` on save) the org's configured SMTP transport, falling back to the global env-configured relay when the org has none/disabled — this activates per-org routing with zero code changes at the ~20 call sites that already pass `organizationId` (invoices, payments, notificationHooks, paymentReminderService, scheduledReportService, taskRunner) but had it silently ignored until now. `auth.js`/`portal.js` password-reset and `authService.js` email-verification sends deliberately stay on the global relay (anti-enumeration timing / no clean org in scope). Adds `email_logs.organization_id` (nullable, backfilled from `clients.organization_id`) via an INFORMATION_SCHEMA-guarded ALTER so the new org-aware sends produce a tenant-attributable audit row; `processQueue()`'s template-only queue path is left global (unchanged) — out of scope, see code comment. Also replaces `POST /bulk/email`'s no-op (`eventBus.emit('bulk.email.queued', ...)` with zero listeners) with a real send: the `{queued, not_found}` response is returned immediately, then — after the response — resolved recipients are emailed via `emailTransport.sendEmail` with bounded concurrency (5 in flight); per-recipient failures are logged, never thrown, and never block other recipients. A server restart mid-batch drops any not-yet-sent recipients with no automatic retry (acceptable first cut — an admin can re-run against the same list).
>

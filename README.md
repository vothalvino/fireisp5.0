...
│   ├── schema.sql           # Combined schema (all 338 tables + column additions)
│   └── migrations/          # Individual numbered migration files (001–441)
...
| `ADMIN_PASSWORD='...' pnpm run admin -- create-user --email admin@example.com --role admin` | Create admin user — the password comes from `ADMIN_PASSWORD`, or is prompted for when run in a terminal. Never pass it as a flag: argv is world-readable via `/proc/<pid>/cmdline` |

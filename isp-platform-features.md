...
- [x] **AI pre-processing on ticket creation**:
  ...
  - [x] AI suggests category, priority, and possible resolution to human agent (stored in `ticket_ai_triage` table; surfaced via `GET /tickets/:id/ai-triage`)
  - [x] AI auto-suggests KB articles and troubleshooting steps based on ticket content (`kb_article_ids` field in `ticket_ai_triage`)
...
- [x] Ticket merging and linking (`POST /tickets/:id/merge` + `ticket_relations` table + `GET/POST/DELETE /tickets/:id/relations`)
- [x] Internal notes (not visible to customer) (`ticket_comments.is_internal` flag, existing)
- [x] File attachment (photos of installation, screenshots) (`files` table with `entity_type='ticket'` + `/api/files/upload`, existing)
- [x] Time logging per ticket (tech work duration) (`ticket_time_logs` table + `GET/POST/PUT/DELETE /tickets/:id/time-logs`)
- [x] **AI-powered ticket summarization**: auto-generate technical summary from conversation thread (`POST /tickets/:id/ai-summary` — degrades gracefully when no LLM key configured)

### 12.2 NOC Dashboard
- [x] Network-wide health status (green/yellow/red) (`GET /noc/health` — device status counts + active alert counts by severity)
- [x] Active alarm count by severity (`GET /noc/alarms` — `alert_events` grouped by severity)
- [x] Ongoing outage map (`GET /noc/outages` — ongoing outages grouped by site; list/grouping, no map dependency added)
...
- [x] Ticket queue by priority and due time (`GET /noc/ticket-queue`)
- [x] Recent events timeline (`GET /noc/events` — last 50 combined alert/outage/ticket events)
- [x] SLA compliance percentage (`GET /noc/sla-compliance` — % non-breached over last 30 days)

### 12.3 Field Operations
- [x] Work order creation and dispatch (`work_orders` table + full CRUD under `/work-orders`)
...
- [x] Route optimization for field visits (`POST /technician-tracking/route-optimize` — nearest-neighbor TSP in pure JS, no external API)
- [x] Material usage logging (cable length, converters, etc.) (`work_order_materials` table + `GET/POST/DELETE /work-orders/:id/materials`)

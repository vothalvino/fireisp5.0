...
- Full customer profile: name, company (RFC/CURP in Mexico), email, phone, address, GPS coordinates
- Service address with map pin + geocoding
- Customer classification: Residential / Business / Corporate / Government
- Customer credit score / risk rating
- Custom fields (unlimited) for technician notes, internal tags, etc.
- Customer photo / ID document upload (INE, passport)
- Family/account grouping (shared billing, family plan)
- Account merging and duplicate detection

### 1.2 Customer Lifecycle
- Lead capture and prospect pipeline
- Service order workflow: request → approval → provisioning → activation
- Automated welcome email/SMS on activation
- Customer onboarding checklist (contract signed, payment method verified, equipment received)
- Contract lifecycle: create, renew, modify, suspend, terminate
- Grace period management with configurable policies
- Win-back campaigns for cancelled customers
- Churn analytics and predictive alerts

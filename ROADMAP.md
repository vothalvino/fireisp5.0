...
- ✅ Multi-tenant support (multiple ISP organizations in one instance) — `POST /api/v1/auth/switch-organization` (validates membership, mints new access token with `orgId` claim, rotates refresh token within the same family); `switchOrganization()` added to frontend `AuthContext`; org switcher dropdown in `Layout.tsx` user area (only shown when `user.organizations.length > 1`)
...
| 2026-04-23 | 5.6 | Redis caching for top 10 endpoints: httpCache(resource, ttl) middleware (cache-aside, version-tag invalidation, X-Cache HIT/MISS header, skips non-GET and non-2xx); bustCache(orgId, resource) increments a per-org per-resource version counter so all cached variants are invalidated atomically; crudController extended with cacheResource option (auto-busts on create/update/partialUpdate/destroy/restore); caching applied to GET /plans (300 s), GET /dashboard/summary (60 s), GET /dashboard/mrr (300 s), GET /dashboard/revenue (300 s), GET /dashboard/device-health (120 s), GET /dashboard/overdue (60 s), GET /clients (60 s), GET /settings (600 s + explicit bust on PUT), GET /nas (300 s), GET /sites (300 s), GET /devices (120 s), GET /reports/financial (300 s), GET /reports/aging (300 s); 16 new Jest tests in httpCache.test.js | #TBD |

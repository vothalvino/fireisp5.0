...
This generalizes beyond limiters `app.js` itself destructures: a limiter destructured only inside an individual *route* file (e.g. `bulk.js` doing `const { bulkEmailLimiter } = require('../middleware/rateLimit')` and using it as `router.post('/email', bulkEmailLimiter, ...)`) breaks the exact same way, because `app.js` mounts every route file transitively. Any test file with a partial rateLimit mock that requires `../src/app` (directly or via requiring another file that requires it) needs the new export added, even though `app.js`'s own top-level `require` call never changed. Seen adding `bulkEmailLimiter`: 7 files needed the addition (identified by checking which ones `require('../src/app')`), `clientDnd.test.js` again didn't.

Also: when a supertest route test mocks `../src/services/cacheService` to make a NEW cacheService-backed check (e.g. a per-org daily budget) deterministic, remember `tenantApiLimiter`'s `CacheStore` (wired into every org-scoped request via `orgScope.js`) *also* calls `cacheService.get`/`.set` on every request, with an unrelated `rl_tenant:*` key. A blanket `expect(cacheService.get).not.toHaveBeenCalled()` will false-fail — scope the assertion to the specific key (`toHaveBeenCalledWith('your:specific:key')` / `not.toHaveBeenCalledWith(...)`) instead.

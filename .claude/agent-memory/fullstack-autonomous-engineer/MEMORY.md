...
- [Auth reset/verify flow](auth-reset-verify-flow.md) — migration 382: users.reset_token_hash etc.; password-reset send is route-layer, register()/resend are service-layer; passwordResetLimiter stacks on existing authLimiter; portal has NO reset mechanism at all; next migration: 383

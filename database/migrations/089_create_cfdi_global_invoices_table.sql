-- Migration: 089_create_cfdi_global_invoices_table
-- Description: CFDI Global (Factura Global) periodic aggregation documents.
--
...
--              through a CFDI Global — a single fiscal document that aggregates
--              all "venta al público en general" transactions for a given period.
--
--              Each row represents one Factura Global issued by an organization
...
--              row for a Factura Global uses one of the organization's público en
--              general clients as the receptor (RFC XAXX010101000).
--
--              Individual invoices belonging to this Factura Global are tracked
--              through the cfdi_global_invoice_items junction table (migration 090).
--
--              This table does NOT apply to global-locale clients — only MX-locale
--              organizations may issue CFDI Global documents.
